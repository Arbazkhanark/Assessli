...
const Contact = ({setShowModal,setContacts}) => {
  ...
  return (
    <div className="bg-black h-[100vh] block inset-0 fixed bg-opacity-30 backdrop-blur-sm z-40">
      <Toaster />
      <div className="text-center border border-gray-500 rounded-lg transition-shadow shadow-gray-200 lg:w-[40%]% md:w-[45%] sm:w-[80%] max-sm:w-[90%] p-2 px-6 m-auto my-40 bg-black">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-x-circle"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>
        ...
        <h1 className="lg:text-3xl md:text-2xl sm:text-xl max-sm:text-lg font-bold text-white m-6">Contact Me</h1>
        ...
        <br />
        <button className="bg-white text-black w-[40%] p-2 sm:w-[100%] max-sm:w-[100%] rounded-lg my-4" onClick={handleSubmit} disabled={loading}>{loading ? "Sending..." : "Submit"}</button>
      </div>
    </div>
  );
}
